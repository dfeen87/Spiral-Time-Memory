...
from theory.dynamics import compare_markov_vs_memory  # noqa: E402
...
def visualize_memory_effect(
    ...
    ax = axes[1]
    ax.plot(
        states_m[:, 0], states_m[:, 1], "b-", linewidth=2, label="Markovian", alpha=0.7
    )
    ax.plot(
        states_mem[:, 0],
        states_mem[:, 1],
    ...
    if config.save_figures:
        ...
        print("✓ Figure saved: memory_effect_comparison.png")
    ...
    print(
        f"Final energy (Markovian): {E_m[-1]:.4f} (conserved: {np.abs(E_m[-1] - E_m[0]) < 0.01})"
    )
    print(
        f"Final energy (Memory): {E_mem[-1]:.4f} (dissipated: {E_m[0] - E_mem[-1]:.4f})"
    )
    print()
...
def compare_energy_dissipation(
    memory_strengths: np.ndarray, F: Callable, x0: np.ndarray, t_final: float = 10.0
) -> Dict[str, np.ndarray]:
    ...
    plt.axhline(
        ...
        linestyle="--",
        linewidth=2,
        label="Markovian (no dissipation)",
        alpha=0.7,
    )
    ...
    print("✓ Figure saved: energy_dissipation_scaling.png")
    print()
...
def plot_phase_space_comparison(
    ...
    fig, axes = plt.subplots(
        1, len(kernel_types) + 1, figsize=(5 * (len(kernel_types) + 1), 5)
    )
    ...
    _, states_m, _ = compare_markov_vs_memory(x0, F, g, config, t_span, dt=0.01)
    ...
    for idx, kernel_type in enumerate(kernel_types):
        ...
        _, _, states = compare_markov_vs_memory(x0, F, g, config, t_span, dt=0.01)
        ...
        ax.plot(x0[0], x0[1], "go", markersize=10, zorder=5)
        ax.set_title(
            f'{kernel_type.replace("_", "-").title()}', fontsize=12, fontweight="bold"
        )
        ax.set_xlabel("Position x")
        ax.grid(True, alpha=0.3)
        ax.set_aspect("equal")
    ...
    print("✓ Figure saved: phase_space_kernel_comparison.png")
    print()
