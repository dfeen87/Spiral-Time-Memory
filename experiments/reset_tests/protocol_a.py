...
class ResetOperation:
    ...
    def __init__(
        self, fidelity: float = 1.0, target_state: Optional[np.ndarray] = None
    ):
        """
    ...
    def apply(self, state: QuantumState, preserve_memory: bool = False) -> QuantumState:
        ...
        reset_rho = self.fidelity * self.target_state + (1 - self.fidelity) * state.rho
...
class HistoryDependenceAnalyzer:
    ...
    def compute_conditional_probability(
        self, outcomes: List[int], histories: List[Tuple[int, ...]], target_outcome: int
    ...
    def test_history_independence(
        ...
        for i, hist in enumerate(unique_histories):
            for j, outcome in enumerate(unique_outcomes):
                mask = [h == hist and o == outcome for h, o in zip(histories, outcomes)]
...
def run_reset_test(config: ResetTestConfig, spiral_time_mode: bool = True) -> Dict:
    """
    ...
    """
    print("Running Protocol A: Reset Test")
    print(
        f"Mode: {'Spiral-Time with Memory' if spiral_time_mode else 'Standard Markovian'}"
    )
    ...
    for cycle in range(config.n_cycles):
        ...
        # Final measurement
        outcome, state = measure_op.measure(
            state, update_memory=False  # Don't update memory in final measurement
        )
    ...
    independence_test = analyzer.test_history_independence(all_outcomes, all_histories)
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print(
        f"Spiral-time history dependence: p={results_spiral['independence_test']['p_value']:.2e}"
    )
    print(
        f"Markovian history dependence: p={results_markov['independence_test']['p_value']:.2e}"
    )
    delta_p = abs(
        results_spiral["independence_test"]["p_value"]
        - results_markov["independence_test"]["p_value"]
    )
    print(f"\nΔp = {delta_p:.2e}")
