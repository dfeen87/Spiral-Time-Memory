...
class ProcessTensor:
    ...
    def add_measurement(
        self, operations: Tuple[int, ...], final_state: np.ndarray, weight: float = 1.0
    ):
        """
...
class CPDivisibilityTest:
    ...
    def test_divisibility(
        process_tensor: ProcessTensor, time_steps: List[int]
    ) -> Dict[str, any]:
        """
        ...
        for t in time_steps[1:]:
            for s in range(1, t):
                ...
                if state_0 is not None and state_s is not None and state_t is not None:
                    ...
                    if not (
                        np.all(eig_s <= eig_0 + 1e-6) and np.all(eig_t <= eig_s + 1e-6)
                    ):
                        violations.append(
                            {
                                "time": t,
...
def reconstruct_process_tensor(
    config: ProcessTensorConfig, use_memory: bool = True
) -> ProcessTensor:
    """
