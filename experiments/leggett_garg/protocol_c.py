...
class LeggettGargInequality:
    ...
    @staticmethod
    def test_violation(K: float, n_times: int = 3, 
        ...
        return {
            ...
            'violation_direction': 'upper' if violates_upper else 'lower' if violates_lower else 'none'
        }


def run_leggett_garg_experiment(
    config: LeggettGargConfig,
    invasive_measurement: bool = True
) -> Dict:
    """
...
def run_protocol_c(config: LeggettGargConfig) -> Dict:
    ...
    if test_with_supp['violates']:
        print("✓ SPIRAL-TIME PREDICTION: Memory suppression does NOT eliminate violation")
    ...
    if test_no_supp['violates'] and test_with_supp['violates']:
        reduction = (test_no_supp['violation_magnitude'] - 
                    test_with_supp['violation_magnitude'])
