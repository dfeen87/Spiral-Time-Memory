...
class SpiralTimeOperator:
    ...
    def effective_hamiltonian(self, t: float, chi_t: float) -> np.ndarray:
    ...
    def von_neumann_evolution(
        ...
        t_final: float,
        chi_trajectory: Callable[[float], float],
        dt: float = 0.01,
    ...
    def check_unitarity(self, rho_ext: np.ndarray) -> bool:
        ...
        return bool(trace_ok and positive_ok)
...
def demonstrate_extended_space():
    ...
    def chi_traj(t: float) -> float:
        return 0.1 * np.sin(2 * np.pi * t)

    # Evolve
    print("Evolving von Neumann equation on H_ext...")
    times, rhos = psi_op.von_neumann_evolution(
        rho_ext_0, t_final=1.0, chi_trajectory=chi_traj, dt=0.01
    )
