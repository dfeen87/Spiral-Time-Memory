...
class NonMarkovianEvolver:
    ...
    def compute_memory_integral(self, t_current: float) -> np.ndarray:
        ...
        if len(self.history_times) == 0:
            ...
            return (
                np.zeros_like(self.history_states[0])
    ...
    def evolve(
        self, x0: np.ndarray, t_span: Tuple[float, float], store_history: bool = True
        ...
        for i, t in enumerate(times[:-1]):
            ...
            if store_history:
                ...
                self.history_states = self.history_states[cutoff_idx:] + [
                    states[i + 1].copy()
                ]
...
def compare_markov_vs_memory(
    ...
    evolver_memory = NonMarkovianEvolver(F=F, g=g, kernel_config=kernel_config, dt=dt)
