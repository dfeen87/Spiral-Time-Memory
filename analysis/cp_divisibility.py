...
def compose_channels(eps1: QuantumChannel, eps2: QuantumChannel) -> QuantumChannel:
    ...
    composed_kraus = [K2 @ K1 for K2 in eps2.kraus_ops for K1 in eps1.kraus_ops]

    return QuantumChannel(composed_kraus)


def test_cp_divisibility(
    channels: List[QuantumChannel], times: List[float]
...
def blp_measure(
    rho0: np.ndarray, channels: List[QuantumChannel], times: List[float]
) -> np.ndarray:
    ...
    for eps in channels:
        ...
        trace_dists_list.append(trace_dist)
...
class ProcessTensorReconstructor:
    ...
    def test_state_independence(
        self, channels: List[QuantumChannel], initial_states: List[np.ndarray]
    ) -> dict:
        ...
        return {
            ...
            "interpretation": (
                ...
                if cv < 0.2
                else "STATE-DEPENDENT (environmental origin likely)"
            ),
        }

    def generate_test_process(
        self, memory_strength: float = 0.0
    ) -> List[QuantumChannel]:
    ...
    def test_memory_hypothesis(
        self, channels: List[QuantumChannel], times: List[float], verbose: bool = True
    ) -> dict:
        ...
        results = {
            ...
            "blp_measure": blp,
            "verdict": "MARKOVIAN (theory falsified)"
            if is_cp_div
            else "NON-MARKOVIAN (consistent with memory)",
        }
